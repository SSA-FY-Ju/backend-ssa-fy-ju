...
- [x] T036 [US5] `service/ConsultationSaveService.java` `saveOrUpdate`에 `@DistributedLock(key = "lock:career-consultation:{sajuResultId}:{yearMonth}")` 적용, 제약조건 이름 문자열 분기 로직 제거
...
- [x] T038 [P] [US6] `SSAju/src/test/java/ssafy/SSAju/service/SajuResultOwnershipTest.java` — 동일 생년월일시의 두 사용자가 같은 `SajuResult.id`를 참조하고 각자 별도의 `UserSajuAccess` 행을 갖는지 확인
- [x] T039 [P] [US6] `SSAju/src/test/java/ssafy/SSAju/service/FeedbackServiceOwnershipTest.java` — `UserSajuAccess` 매핑이 없는 사용자의 피드백/상담 접근이 거부되는지 확인
- [x] T040 [P] [US6] `SSAju/src/test/java/ssafy/SSAju/service/CareerFortuneQuotaTest.java` — 정본 최초 생성이 일일 쿼터를 차감하지 않는지 확인

### Implementation for User Story 6

- [x] T041 [US6] `career/entity/SajuResult.java` 수정 — `user`(`@ManyToOne` FK `user_id`) 필드 제거, 기존 복합 유니크(`user_id, user_profile_id`) 대신 `user_profile_id` 단독 유니크로 변경
- [x] T042 [US6] `career/entity/UserSajuAccess.java` 신규 생성 — `id`, `user`(FK, LAZY), `sajuResult`(FK, LAZY), `createdAt`(`@CreatedDate`), 유니크 `(user_id, saju_result_id)` (Lombok 규칙: `@Getter`+`@NoArgsConstructor(PROTECTED)`+`@Builder`, equals/hashCode ID 수기)
- [x] T043 [P] [US6] `repository/UserSajuAccessRepository.java` 신규 생성 — `existsByUserIdAndSajuResultId` (`findByUserId` 등은 현재 어떤 호출자도 필요로 하지 않아 죽은 코드가 되므로 생략 — F 원칙)
- [x] T044 [US6] (Git 커밋 대상 제외, 스크래치패드에만 생성) 운영 DB 마이그레이션 스크립트 작성 — 중복 정본 그룹별 생존 정본 선정 → 하위 테이블(`career_consultation` 등)의 `saju_result_id` UPDATE → `UserSajuAccess` 백필 INSERT → 중복 행 삭제 → `user_id` 컬럼/제약 DROP → `user_profile_id` 단독 유니크 생성 (data-model.md 마이그레이션 순서 그대로 반영, **이 파일은 git add 대상에서 제외**)
- [x] T045 [US6] `service/CareerFortuneService.java` 수정 — Quota 차감 로직 완전 제거. *(계획 대비 위치 조정: 정본 최초 생성/최초 접근 시 `UserSajuAccessRepository` 매핑 생성은 `CareerFortuneService`가 아니라 `SajuResultProvider.findOrCreate`에 구현 — `ConsultationService`도 동일하게 `findOrCreate`를 호출하므로 공통 진입점에 둬야 두 플로우 모두에서 접근 매핑이 보장됨)*
- [x] T046 [US6] `service/FeedbackService.java` 수정 — 기존 `SajuResult.user_id` 기반 소유권 검증을 `UserSajuAccessRepository.existsByUserIdAndSajuResultId` 기반으로 전면 교체. *(구현 위치: `CareerConsultationRepository.findByIdAndAccessibleByUser`가 `UserSajuAccess` EXISTS 서브쿼리로 DB 레벨 검증을 수행하고 `FeedbackService`는 이를 호출 — C-4 원칙(lazy loading 체인 대신 DB JOIN) 유지)*
- [x] T046-1 [US6] (선행 마이그레이션) `user_satisfaction_feedback` 테이블의 `feedback_type` 컬럼 데이터 마이그레이션 — *옵션 1(완전한 사전 변환) 채택: 코드에 하위호환 AttributeConverter를 남기지 않고, 배포 전 실행할 UPDATE 3종을 스크래치패드 마이그레이션 스크립트에 추가(`CAREER_TIMING→SAJU`, `CONSULTATION→CAREER_CONSULTATION`, `COMPATIBILITY→COMPANY_COMPATIBILITY`). 이 UPDATE는 T047 코드 배포보다 반드시 먼저 실행되어야 함 — 순서를 지키지 않으면 레거시 값을 AnalysisType으로 역직렬화할 때 즉시 예외가 발생함(git 커밋 범위 밖)*
- [x] T047 [US6] `career/enums/FeedbackType.java` 삭제 및 다음 참조 파일들을 `career/enums/AnalysisType.java`로 치환(매핑: `CAREER_TIMING→SAJU`, `CONSULTATION→CAREER_CONSULTATION`, `COMPATIBILITY→COMPANY_COMPATIBILITY`): `career/entity/UserSatisfactionFeedback.java`, `admin/controller/AdminFeedbackController.java`, `admin/service/AdminFeedbackService.java`, `admin/dto/FeedbackListDTO.java`, `admin/repository/AdminFeedbackQueryRepository.java`(`admin/dto/FeedbackStatDTO.java`는 `Map<String,Long>`만 다뤄 FeedbackType 직접 참조가 없어 변경 불필요 확인). `templates/admin/feedback-management.html`의 리터럴 문자열(뱃지/필터/차트 키)도 함께 갱신. **C1 선행 결합**: 같은 작업 단위에서 `controller/FeedbackController.java`→`career/controller/FeedbackController.java`, `dto/request/SatisfactionFeedbackRequest.java`→`career/dto/request/SatisfactionFeedbackRequest.java`, `dto/response/SatisfactionFeedbackResponse.java`→`career/dto/response/SatisfactionFeedbackResponse.java`로 함께 이동한다(패키지 이동과 Enum 치환을 서로 다른 Phase에서 나눠 하면 동일 파일에 대한 "수정 후 이동"으로 git 이력이 흩어지고 중간 빌드 리스크가 커지므로, 피드백 관련 파일만 여기서 선제적으로 이동 — 나머지 C1 대상은 Phase 11에서 그대로 진행)
