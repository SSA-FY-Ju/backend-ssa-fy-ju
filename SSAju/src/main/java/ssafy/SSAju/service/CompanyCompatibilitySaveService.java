 ...
 *
 * <p>그럼에도 임대시간 만료를 완전히 배제할 수는 없으므로(DB 커넥션 풀 고갈 등 극단적 상황),
 * 실제 삽입은 {@link CompanyCompatibilityInsertService}의 REQUIRES_NEW 트랜잭션으로 분리해
 * UNIQUE 제약 위반 시 안전하게 재조회로 복구한다({@code ConsultationSaveService}와 동일한 이유).
 *
 * <p>같은 클래스 내 self-invocation에서는 Spring AOP 프록시가 {@code @DistributedLock}을
 * 가로채지 못하므로({@code ConsultationSaveService}/{@code SajuResultWriteService}와 동일한
 * 이유), {@code CompanyMatchingService}와 분리된 별도 빈으로 존재한다.
 *
...
public class CompanyCompatibilitySaveService {
    ...
    private final CompanyCompatibilityInsertService companyCompatibilityInsertService;
    ...
    public CompanyCompatibility saveWithLock(CompanyCompatibility entity, CompatibilityAnalysisData analysisData) {
        ...
        try {
            CompanyCompatibility saved = companyCompatibilityInsertService.insert(entity, analysisData);
            ...
            log.warn("CompanyCompatibility 삽입 중 UNIQUE 제약 위반(락 임대시간 만료 경합 추정) — 재조회: "
                    + "userProfileId={}, month={}", entity.getUserProfile().getId(), entity.getCompatibilityMonth());
