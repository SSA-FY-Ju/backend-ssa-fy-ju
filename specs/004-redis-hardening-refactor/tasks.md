...
- [ ] T022-1 [P] [US2] 통합/단위 테스트 `SSAju/src/test/java/ssafy/SSAju/service/ConsultationServiceQuotaTest.java` — `ConsultationService.getCareerConsultation`에서 캐시 미스 후 OpenAI 호출이 실패(타임아웃/`RestClientException`/응답 검증 실패)하면 쿼터가 요청 전과 동일하게 복원되는지, 캐시 히트 시에는 애초에 차감되지 않는지, 성공 시 정확히 1 차감되는지 확인
- [ ] T022-2 [P] [US2] 단위 테스트 `SSAju/src/test/java/ssafy/SSAju/career/caller/ConsultationOpenAICallerRetryTest.java` — `ConsultationOpenAICaller.call`이 실제로 던지는 타임아웃 예외의 구체 타입(Spring AI `ChatClient`가 하위 `RestClient`의 `ResourceAccessException`을 그대로 전파하는지, 자체 예외로 재감싸는지 확인 — 2026-07-12 운영 로그의 `statusCode=0`/"재시도 불가" 증상이 현재 catch-all(`catch(Exception e)`) 분기와 일치해 재현 가능성이 있음), 재시도 횟수(`maxAttempts=3`, backoff 적용 여부), 4xx 응답은 재시도 없이 즉시 `OpenAIApiException`으로 실패하는지를 검증한다
...
- [ ] T024-2 [P] [US2] `caller/ConsultationOpenAICaller.java` 수정(원인 규명 후속 조치) — T022-2에서 확인된 실제 타임아웃 예외 타입을 기준으로 `@Retryable`의 `retryFor`/`noRetryFor`를 조정한다. 현재도 `retryFor = {ResourceAccessException.class, HttpServerErrorException.class}`, `noRetryFor = {OpenAIApiException.class, HttpMessageConversionException.class}`가 설정돼 있고 `catch` 블록도 이 셋을 구분하고 있지만, Spring AI `ChatClient`가 하위 `RestClient`의 예외를 그대로 전파하지 않고 별도 타입으로 재감싸는 경우 `catch(ResourceAccessException)`에 잡히지 않고 catch-all(`catch(Exception e)`, "재시도 불가") 경로로 떨어질 수 있다 — T022-2 테스트로 실제 타입을 먼저 확인한 뒤에만 `retryFor`/`catch` 목록에 해당 타입을 추가할 것(4xx 계열은 여전히 `noRetryFor`/`OpenAIApiException` 변환으로 재시도 차단 유지). T024-1(보상 로직)과 별개로 실패 자체의 빈도를 줄이는 조치이므로 두 태스크 모두 필요
